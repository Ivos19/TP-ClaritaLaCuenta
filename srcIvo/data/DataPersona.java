...
public class DataPersona {
	...
	public Persona getByDni(Persona per) throws Exception{
		...
		try {
			stmt=FactoryConexion.getInstancia().getConn().prepareStatement(
					"select p.id, nombre, apellido, dni, habilitado, categoriaId, user, pass, descripcion from persona p inner join categoria c on p.categoriaId=c.id where dni=?");
	...
	public void add(Persona p) throws Exception{
		...
		try {
			stmt=FactoryConexion.getInstancia().getConn()
					.prepareStatement(
					"insert into persona(dni, nombre, apellido, habilitado, categoriaId, user, pass) values (?,?,?,?,?,?,?)",
	...
	public void update(Persona p) throws Exception{
		...
		try {
			stmt=FactoryConexion.getInstancia().getConn()
					.prepareStatement(
					"update persona set dni=?, nombre=?, apellido=?, habilitado=?, categoriaId=?, user=?, pass=? where id=?"
					);
			stmt.setString(1, p.getDni());
			stmt.setString(2, p.getNombre());
			stmt.setString(3, p.getApellido());
	...
	public Persona getById(int id) throws Exception {
		...
		try {
			stmt=FactoryConexion.getInstancia().getConn().prepareStatement(
					"select p.id, nombre, apellido, dni, habilitado, categoriaId, user, pass, descripcion from persona p inner join categoria c on p.categoriaId=c.id where p.id=?");
			stmt.setInt(1, id);
			rs=stmt.executeQuery();
			if(rs!=null && rs.next()){
					p=new Persona();
